...
Ext.define('Trackmont.controller.Root', {
    extend: 'Ext.app.Controller',
    alias: 'controller.root',

    requires: [
        'Trackmont.view.dialog.Login',
        'Trackmont.view.Main',
        'Trackmont.view.MainMobile',
    ...
    init: function () {
        ...
        for (i = 0; i < data.length; i++) {
            attribute = data[i];
            Trackmont.model.Position.addFields([{
    ...
    calculateAttribute: function (data) {
        ...
        if (value !== undefined) {
            return Trackmont.AttributeFormatter.getAttributeConverter(this.attributeKey)(value);
        }
        return null;
    },
    ...
    onServerReturn: function (options, success, response) {
        ...
        if (success) {
            Trackmont.app.setServer(Ext.decode(response.responseText));
    ...
    asyncUpdate: function (first) {
        ...
        socket.onclose = function () {
            Trackmont.app.showToast(Strings.errorSocket, Strings.errorTitle);
            ...
            }, Trackmont.Style.reconnectTimeout);
        };
    ...
    updatePositions: function (array, first) {
        ...
        for (i = 0; i < array.length; i++) {
            ...
            if (entity) {
                ...
                store.add(Ext.create('Trackmont.model.Position', array[i]));
            }
            if (Ext.getStore('Events').findRecord('positionId', array[i].id, 0, false, false, true)) {
                Ext.getStore('EventPositions').add(Ext.create('Trackmont.model.Position', array[i]));
            }
        }
        if (first) {
    ...
    updateEvents: function (array) {
        ...
        for (i = 0; i < array.length; i++) {
            ...
            if (device) {
                ...
                Trackmont.app.showToast(array[i].text, device.get('name'));
            }
        }
    },
    ...
    zoomToAllDevices: function () {
        var lat, lon, zoom;
        lat = Trackmont.app.getPreference('latitude', 0);
        lon = Trackmont.app.getPreference('longitude', 0);
        zoom = Trackmont.app.getPreference('zoom', 0);
    ...
    initReportEventTypesStore: function () {
        ...
        store.add({
            type: Trackmont.store.ReportEventTypes.allEvents,
            name: Strings.eventAll
        });
        Ext.create('Trackmont.store.AllNotifications').load({
            scope: this,
            callback: function (records, operation, success) {
                ...
                if (success) {
                    for (i = 0; i < records.length; i++) {
                        ...
                        store.add({
                            type: value,
                            name: Trackmont.app.getEventString(value)
                        });
                    }
                }
            }
