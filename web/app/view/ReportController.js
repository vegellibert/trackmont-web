...
Ext.define('Trackmont.view.ReportController', {
    ...
    requires: [
        ...
        'Trackmont.model.ReportTrip',
        'Trackmont.view.dialog.ReportConfig',
    ...
    hideReports: function () {
        Trackmont.app.showReports(false);
    },
    ...
    init: function () {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            this.routeColumns.push({
                ...
                renderer: Trackmont.AttributeFormatter.getAttributeFormatter(attribute.get('key')),
                hidden: true
            });
        }
        if (Trackmont.app.getVehicleFeaturesDisabled()) {
    ...
    onConfigureClick: function () {
        var dialog = Ext.create('Trackmont.view.dialog.ReportConfig');
        ...
        if (this.eventType !== undefined) {
            ...
            dialog.lookupReference('eventTypeField').setValue([Trackmont.store.ReportEventTypes.allEvents]);
        }
    ...
    onSelectionChange: function (selection, selected) {
        ...
        if (selected.length > 0) {
            ...
            if (report instanceof Trackmont.model.ReportTrip) {
                this.selectTrip(report);
            }
            if (report instanceof Trackmont.model.Event || report instanceof Trackmont.model.ReportStop) {
    ...
    selectReport: function (object) {
        ...
        if (object instanceof Trackmont.model.Position) {
            if (reportType === 'route') {
        ...
        if (this.lookupReference('reportTypeField').getValue() === 'chart') {
            ...
            for (i = 0; i < deviceIds.length; i++) {
                ...
                chartSeries.push({
                    ...
                    xField: 'fixTime',
                    highlightCfg: {
                        scaling: Trackmont.Style.chartMarkerHighlightScaling
                    },
                    colors: [Trackmont.app.getReportColor(deviceIds[i])],
                    marker: {
                        type: 'circle',
                        radius: Trackmont.Style.chartMarkerRadius,
                        fill: Trackmont.app.getReportColor(deviceIds[i])
                    }
                });
            }
                    ...
                    if (records.length > 0) {
                        if (!records[0].get('positionId')) {
                            if (Trackmont.app.isMobile()) {
                                Trackmont.app.showReports(true);
                            } else {
                                this.getView().expand();
                            }
                        }
    ...
    routeColumns: [{
        ...
        dataIndex: 'deviceId',
        renderer: Trackmont.AttributeFormatter.getFormatter('deviceId')
    }, {
        text: Strings.positionValid,
        dataIndex: 'valid',
        renderer: Trackmont.AttributeFormatter.getFormatter('valid')
    }, {
        text: Strings.positionFixTime,
        dataIndex: 'fixTime',
        xtype: 'datecolumn',
        renderer: Trackmont.AttributeFormatter.getFormatter('fixTime')
    }, {
        text: Strings.positionLatitude,
        dataIndex: 'latitude',
        renderer: Trackmont.AttributeFormatter.getFormatter('latitude')
    }, {
        text: Strings.positionLongitude,
        dataIndex: 'longitude',
        renderer: Trackmont.AttributeFormatter.getFormatter('longitude')
    }, {
        text: Strings.positionAltitude,
        dataIndex: 'altitude',
        renderer: Trackmont.AttributeFormatter.getFormatter('altitude')
    }, {
        text: Strings.positionSpeed,
        dataIndex: 'speed',
        renderer: Trackmont.AttributeFormatter.getFormatter('speed')
    }, {
        text: Strings.positionAddress,
        dataIndex: 'address',
        renderer: Trackmont.AttributeFormatter.getFormatter('address')
    }],
    ...
    eventsColumns: [{
        ...
        xtype: 'datecolumn',
        renderer: Trackmont.AttributeFormatter.getFormatter('serverTime')
    }, {
        text: Strings.reportDeviceName,
        dataIndex: 'deviceId',
        renderer: Trackmont.AttributeFormatter.getFormatter('deviceId')
        ...
        renderer: Trackmont.AttributeFormatter.getFormatter('geofenceId')
    }],
    ...
    summaryColumns: [{
        ...
        dataIndex: 'distance',
        renderer: Trackmont.AttributeFormatter.getFormatter('distance')
    }, {
        ...
        renderer: Trackmont.AttributeFormatter.getFormatter('duration')
    }, {
        text: Strings.reportSpentFuel,
        dataIndex: 'spentFuel',
        renderer: Trackmont.AttributeFormatter.getFormatter('spentFuel')
    }],
    ...
    tripsColumns: [{
        ...
        dataIndex: 'startTime',
        xtype: 'datecolumn',
        renderer: Trackmont.AttributeFormatter.getFormatter('startTime')
    }, {
        ...
        dataIndex: 'endTime',
        xtype: 'datecolumn',
        renderer: Trackmont.AttributeFormatter.getFormatter('endTime')
    }, {
        ...
        renderer: Trackmont.AttributeFormatter.getFormatter('driverUniqueId')
    }],
