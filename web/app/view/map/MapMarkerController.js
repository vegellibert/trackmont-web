...
Ext.define('Trackmont.view.map.MapMarkerController', {
    ...
    init: function () {
        ...
        this.liveRoutes = {};
        this.liveRouteLength = Trackmont.app.getAttributePreference('web.liveRouteLength', 10);
        this.selectZoom = Trackmont.app.getAttributePreference('web.selectZoom', 0);
    },
    ...
    getAreaStyle: function (label, color) {
        ...
        if (color) {
            ...
            strokeColor = Trackmont.Style.mapGeofenceColor;
        }
        ...
        if (label) {
            styleConfig.text = new ol.style.Text({
                text: label,
                fill: new ol.style.Fill({
                    color: Trackmont.Style.mapGeofenceTextColor
                }),
                stroke: new ol.style.Stroke({
                    color: Trackmont.Style.mapTextStrokeColor,
                    width: Trackmont.Style.mapTextStrokeWidth
                }),
                font: Trackmont.Style.mapTextFont
            });
        }
    ...
    getDeviceColor: function (device) {
        switch (device.get('status')) {
            case 'online':
                return Trackmont.Style.mapColorOnline;
            case 'offline':
                return Trackmont.Style.mapColorOffline;
            default:
                return Trackmont.Style.mapColorUnknown;
        }
    },
    ...
    updateAccuracy: function (position, device) {
        ...
        if (position.get('accuracy')) {
            ...
            if (feature) {
                ...
                feature.setStyle(this.getAreaStyle(null, Trackmont.Style.mapAccuracyColor));
    ...
    clearReport: function () {
        ...
        if (this.selectedMarker && !this.selectedMarker.get('event') &&
                this.selectedMarker.get('record') instanceof Trackmont.model.Position) {
    ...
    getRouteStyle: function (deviceId) {
        return new ol.style.Style({
            stroke: new ol.style.Stroke({
                color: Trackmont.app.getReportColor(deviceId),
                width: Trackmont.Style.mapRouteWidth
            })
        });
    },

    getMarkerStyle: function (zoom, color, angle, category) {
        var image = Trackmont.DeviceImages.getImageIcon(color, zoom, angle, category);
        return new ol.style.Style({
            image: image,
            text: new ol.style.Text({
                textBaseline: 'bottom',
                fill: new ol.style.Fill({
                    color: Trackmont.Style.mapTextColor
                }),
                stroke: new ol.style.Stroke({
                    color: Trackmont.Style.mapTextStrokeColor,
                    width: Trackmont.Style.mapTextStrokeWidth
                }),
                offsetY: -image.getSize()[1] / 2 - Trackmont.Style.mapTextOffset,
                font: Trackmont.Style.mapTextFont
            })
        });
    },
    ...
    getReportMarker: function (deviceId, angle) {
        return this.getMarkerStyle(false, Trackmont.app.getReportColor(deviceId), angle, 'arrow');
    },

    resizeMarker: function (style, zoom) {
        var image, text;
        image = Trackmont.DeviceImages.getImageIcon(
            style.getImage().fill, zoom, style.getImage().angle, style.getImage().category);
        text = style.getText();
        text.setOffsetY(-image.getSize()[1] / 2 - Trackmont.Style.mapTextOffset);
        style.setText(text);
        style.setImage(image);
    },

    rotateMarker: function (style, angle) {
        style.setImage(Trackmont.DeviceImages.getImageIcon(
    ...
    selectFeature: function (feature) {
        ...
        if (record) {
            if (record instanceof Trackmont.model.Device) {
