...
Ext.define('Trackmont.view.map.MapController', {
    extend: 'Trackmont.view.map.MapMarkerController',
    ...
    init: function () {
        ...
        this.lookupReference('showReportsButton').setVisible(
            Trackmont.app.isMobile() && !Trackmont.app.getBooleanAttributePreference('ui.disableReport'));
        this.lookupReference('showEventsButton').setVisible(Trackmont.app.isMobile());
    },
        ...
        if (this.lookupReference('showGeofencesButton').pressed) {
            Ext.getStore('Geofences').each(function (geofence) {
                var feature = new ol.Feature(
                    Trackmont.GeofenceConverter.wktToGeometry(this.getView().getMapView(), geofence.get('area')));
